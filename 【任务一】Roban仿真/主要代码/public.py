...
POSE_TOPIC = "/sim/torso/PR"
# POSE_TOPIC = "/initialpose"
STEP_LEN_MAX = [0.05, 0.05, 10]
# STEP_LEN_MAX = [0.03, 0.02, 10]
...
class PublicNode(bodact.Action):
    ...
    #     super(PublicNode, self).__init__()
        ...
        rospy.Subscriber(POSE_TOPIC, Float64MultiArray, self.pose_callback)
    ...
    def pose_callback(self, msg):
        roban_torsoPR = msg.data 
        x, y = roban_torsoPR[0], roban_torsoPR[1]
        yaw=roban_torsoPR[9]
        # _, _, yaw = self.toRPY(msg.pose.pose)
        old_pos=self.current_pose
        self.current_pose = [x, y, yaw * 180.0 / math.pi]
        if self.__debug:
            print ('pose:', self.current_pose)
        if ((old_pos[0]-self.current_pose[0])**2+(old_pos[1]-self.current_pose[1])**2)>=1 or abs(old_pos[2]-self.current_pose[2])>50:
            return#滤掉高频跳点
